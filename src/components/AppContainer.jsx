  ...
  return (
    <React.Fragment>
      <Container maxWidth="md">
        <header>
          <h1>
            <img src="/assets/images/header-image.png" alt={process.env.REACT_APP_TITLE} />
          </h1>
          <div className="langs">
            {lang !== 'tr' && (
        ...
        <div>
          <Tabs value={value} onChange={handleChange} centered>
            <Tab
              {...a11yProps(0)}
              value={0}
              label={<div dangerouslySetInnerHTML={{ __html: getLang('tabs')[0] }}></div>}
            />
            <Tab
              {...a11yProps(1)}
              value={1}
              label={<div dangerouslySetInnerHTML={{ __html: getLang('tabs')[1] }}></div>}
            />
            <Tab
              {...a11yProps(2)}
              value={2}
              label={<div dangerouslySetInnerHTML={{ __html: getLang('tabs')[2] }}></div>}
            />
            <Tab
              {...a11yProps(3)}
              value={3}
              label={<div dangerouslySetInnerHTML={{ __html: getLang('tabs')[3] }}></div>}
            />
          </Tabs>
      ...
      <Typography
        variant="h4"
        style={{ marginTop: '1rem', fontSize: '1rem', opacity: '0.4' }}
        align="center"
      >
        <a
          ...
          target="_blank"
          rel="noreferrer"
          title="Web Developer Ferdi Tarakçı"
        >
          Development: Ferdi Tarakçı
        </a>
      </Typography>
    </React.Fragment>
  )
