...
const Note = "NOT: Enerji harcaması için bulunan değer ortalama bir değer olup, bireyin vücut tipi, yaptığı aktivitenin şiddeti, yaşı vb. bireysel faktörlere göre değişiklik gösterebilir. Kaynak: Maughan R, Nutrition in Sport, 2001, Blackwell Science Ltd."
...
const EnergySpentByActivity = () => {
	...
	const handleSubmit = async (e) => {
		...
		let isValidate = (obj.activityGroup === 0 || obj.activity === 0 || obj.weight === "" || obj.activityDuration === "")
	...
	}, [form.activityGroup, form, activities])
			...
			<Grid container spacing={1.5}>
				<Grid item xs={6} sm={6} md={2.5}>
					<FormControl fullWidth error={!form.activityGroup && form.activityGroup === 0}>
						...
						>
							...
							{activities.map(({ name }, index) => <MenuItem value={name} key={index}>{name}</MenuItem>)}
						</Select>
				...
				<Grid item xs={6} sm={6} md={2.5}>
					<FormControl fullWidth error={!form.activity && form.activity === 0}>
						...
						>
							<MenuItem value={form.gender === "" ? "" : 0}>{getLang('choose')}</MenuItem>
							{activityList.map(({ title, value }, index) => <MenuItem value={value} key={index}>{title}</MenuItem>)}
						</Select>
						{!form.gender && form.gender === 0 && <FormHelperText style={{ marginLeft: 0 }}>{getLang('required')}</FormHelperText>}
					</FormControl>
				</Grid>
				<Grid item xs={6} sm={4} md={3}>
					<FormControl fullWidth>
						...
						/>
						{!form.activityDuration && form.activityDuration !== null && <FormHelperText error style={{ marginLeft: 0 }}>{getLang('required')}</FormHelperText>}
					</FormControl>
				</Grid>
				...
				<Grid item xs={12}>
					...
					{result.title && <Typography variant="h2" style={{ fontSize: "3rem", marginBottom: "1rem", color: "var(--orange)" }} align="center">{result.title}</Typography>}
					{result.description && (<Typography align="center">{result.description}</Typography>)}
					{result.description && (<Typography align="center" style={{ marginTop: "2rem" }}><Typography variant="caption">{Note}</Typography></Typography>)}
				</Grid>
			</Grid>
		</form>
	)
